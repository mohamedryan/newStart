...
$(document).ready(function() {
	var viewport = $('#wrapper');
	var navigationSystem = $('#slide-out a,.brand-logo');
	// creating loading animation 
	var loading = $('<div id="main-progress"> <div class="wrapper"> <p>Getting your content ...</p><div class="progress"><div class="indeterminate"></div></div></div></div>'); 
	var sendAgaxRequest = function (to) {
		viewport.velocity({
				opacity: 0,
			}, {
				display: 'none',
				...
				complete: function () {
					ajaxContentLoad(to);
					viewport.after(loading.velocity({opacity: 1}, {display: 'block'}))
				},
	...
	// on browser's default (back and forward buttons) --- event listener
	window.onpopstate = function () {
		sendAgaxRequest(location.href)
	};
	
	navigationSystem.on('click', function (e) {
		e.preventDefault();
		var anchor = $(this);
		var url = anchor.attr('href');
		var urlOpject = getLocation(url);

		// do nothing if there is already an ajax request  or if this page is already displayed  
		if (anchor.hasClass('disabled') || anchor.hasClass('active')) {
			return false;
		}

		// set all link disabled 
		navigationSystem.addClass('disabled');
		// remove active claass from all the navigation links 
		navigationSystem.removeClass('active');
		// mark the clicked anchor
		anchor.addClass('active');
		
		// changing the url 3:)
			/* note
				should be after the ajax request
			*/
		history.pushState({},'',urlOpject.pathname);
	...
	function ajaxContentLoad(url) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				// turn ajax response data (html as string) to jQuery opject 
				var htmlData = $(data);
				// get new page title
				var pageTitle = htmlData.find('title').text();
				// get the content that will be displayed 
				var content = htmlData.find("#wrapper").html();

				viewport.html(content);

				// hide the loading progress animation
				loading.velocity('stop').velocity({opacity: 0}, {
					display: 'none',
					// call back after hiding the loading progress animation
					complete: function () {
						// remove it
						loading.remove();

						// and thin display the content nicely :) ^_^ 
						viewport.velocity('stop').velocity({opacity: 1},{
							display: 'block',
							duration: 150,
							// call back after the content display animation is done and the the whole ajax requeest as well
							complete: function () {
								// return all nav links to work agin (not to be disabled any more)
								navigationSystem.removeClass('disabled')
	...
	// make a virutal anchor to turn it's url to an url opject !!!
	function getLocation (href) {
		var a = $('<a href='+href+'></a>');
		return a[0];
	}

	// made by ♥ and alote of anise  😂😂😂😂
});
